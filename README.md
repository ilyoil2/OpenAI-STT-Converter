...
```bash
./scripts/download-video "YOUTUBE_URL"
```

영상 일부만 다운로드

```bash
./scripts/download-video "YOUTUBE_URL" "00:00" to "1:01:00"
```

일본어 Small:

```bash
./scripts/transcribe-ja-small "YOUTUBE_URL"
```

일본어 Medium:

```bash
./scripts/transcribe-ja-medium "YOUTUBE_URL"
```

영어 Small:

```bash
./scripts/transcribe-en-small "YOUTUBE_URL"
```

영어 Medium:

```bash
./scripts/transcribe-en-medium "YOUTUBE_URL"
```
...
```bash
./scripts/transcribe-ja-small "YOUTUBE_URL" -o saves/custom_name.srt
```
...
```bash
./scripts/transcribe-ja-small "YOUTUBE_URL" --keep-audio
```
